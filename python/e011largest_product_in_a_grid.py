...
class InsufficientGridSizeError(Exception):
    pass
...
def largest_grid_product(raw_grid, n):
    ...
    # First, create 2-dimensional array of grid called 'g'
    g = set_grid(raw_grid)

    # Next, determine dimensions of grid
    height = len(g)
    width = len(g[0])

    # Raise error if grid is smaller than specified number of adjacents
    ...
    max_r_diag_prod = largest_right_diag_product(g, n)

    return max([max_row_prod, max_col_prod, max_l_diag_prod, max_r_diag_prod])
...
def largest_row_product(grid, n):
    ...
    for i in range(height):
        ...
        for j in range(width - n + 1):
            local_product = 1
            for k in range(n):
                local_product *= int(grid[i][j + k])
            if local_product > ret:
                ret = local_product
    return ret
...
def largest_left_diag_product(grid, n):
    ...
    for i in range(height - n + 1):
        ...
        for j in range(width - n + 1):
            local_product = 1
            for k in range(n):
                local_product *= int(grid[i + k][j + k])
            if local_product > ret:
                ret = local_product
    return ret
...
def largest_right_diag_product(grid, n):
    ...
    for i in range(height - n + 1):
        ...
        for j in range(width - n + 1):
            local_product = 1
            for k in range(n):
                local_product *= int(grid[(i + n - 1) - k][j + k])
            if local_product > ret:
                ret = local_product
    return ret
...
if __name__ == '__main__':
    our_n = 4
    print('Largest product of {} adjacent numbers: {}'.format(
        our_n, largest_grid_product(EULER_GRID, our_n)))
